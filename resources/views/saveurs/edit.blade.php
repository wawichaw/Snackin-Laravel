...
@section('title', __('Modifier la saveur - Snackin'))
  ...
  <div class="saveurs-content">
    <div class="container" style="max-width: 800px; margin: 0 auto; padding: 40px 20px;">
      <div class="saveur-header">
        <h1 class="saveur-title">✏️ {{ __('Modifier la Saveur') }}</h1>
        <p class="saveur-subtitle">{{ __('Modifiez les informations de cette saveur') }}</p>
      </div>
      ...
      <div class="saveur-form-container">
        <form action="{{ route('saveurs.update', $saveur) }}" method="post">
          ...
          <div class="form-group-saveur">
            ...
            <input type="text" id="nom_saveur" name="nom_saveur" value="{{ old('nom_saveur', $saveur->nom_saveur) }}" required placeholder="{{ __('Ex: Chocolat, Vanille, Pistache...') }}">
          </div>

          <div class="form-group-saveur">
            <label for="emoji">😊 {{ __('Emoji') }}</label>
            <input type="text" id="emoji" name="emoji" value="{{ old('emoji', $saveur->emoji) }}" placeholder="{{ __('Ex: 🍫, 🍓, 🥜...') }}">
          ...
          <div class="form-group-saveur">
            ...
            <textarea id="description" name="description" placeholder="{{ __('Décrivez cette saveur, ses caractéristiques, son goût...') }}">{{ old('description', $saveur->description) }}</textarea>
          </div>
