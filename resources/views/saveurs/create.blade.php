...
@section('title', 'Nouvelle saveur - Snackin')
  ...
  <div class="saveurs-content">
    <div class="container" style="max-width: 800px; margin: 0 auto; padding: 40px 20px;">
      <div class="saveur-header">
        <h1 class="saveur-title">✨ Nouvelle Saveur</h1>
        <p class="saveur-subtitle">Créez une nouvelle saveur pour enrichir votre collection de biscuits</p>
      </div>
      ...
      <div class="saveur-form-container">
        <form action="{{ route('saveurs.store') }}" method="post">
          ...
          <div class="form-group-saveur">
            ...
            <input type="text" id="nom_saveur" name="nom_saveur" value="{{ old('nom_saveur') }}" required placeholder="Ex: Chocolat, Vanille, Pistache...">
          </div>

          <div class="form-group-saveur">
            <label for="emoji">😊 Emoji</label>
            <input type="text" id="emoji" name="emoji" value="{{ old('emoji') }}" placeholder="Ex: 🍫, 🍓, 🥜...">
            <small style="color: var(--saveur-muted); font-size: 14px;">Ajoutez un emoji pour rendre la saveur plus attractive</small>
          </div>

          <div class="form-group-saveur">
            <label for="description">📝 Description</label>
            <textarea id="description" name="description" placeholder="Décrivez cette saveur, ses caractéristiques, son goût...">{{ old('description') }}</textarea>
          </div>
